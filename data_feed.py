...
# ============================================================
# BingX Market API (دقیقاً مطابق داده‌های اسپات TradingView)
# ============================================================
...
def load_from_bingx(
    ...
    """
    BingX Historical K-line API - اندپوینت رسمی و فعال برای دریافت داده‌های کندل.
    این اندپوینت دقیقاً همان داده‌هایی را برمی‌گرداند که TradingView برای BINGX:DASHUSDT نشان می‌دهد.
    """
    # آدرس صحیح و به‌روز مستندات BingX
    url = "https://open-api.bingx.com/openApi/market/his/v1/kline"
    
    interval = "1d"
    limit = 500  # حداکثر مقدار مجاز در این اندپوینت 500 است
    all_rows = []
    end_ms: Optional[int] = None
    
    # در BingX نماد باید با خط تیره و حروف بزرگ باشد: DASH-USDT
    ...
    while len(all_rows) < n_bars:
        ...
        # بررسی کد پاسخ (0 یعنی موفقیت‌آمیز)
        ...
        # هر row یک آرایه است: [timestamp, open, high, low, close, volume]
...
def load_market_data(
    ...
    # اولویت ۲: BingX Market API (دقیقاً همان داده‌های TradingView)
    try:
